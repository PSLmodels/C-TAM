...
# This script imputes Housing Choice Voucher Program (HCV) and Section 8 project-based rental assistance recipients dollar benefit amount to match the aggregates with United States Department of Housing and Urban Development (HUD) statistics for HCV. In this current version, we used 2014 CPS data and USDA FY2014 annual reports on SNAP. Please refer to the documentation in the same folder for more details on methodology and assumptions. The output this script is a personal level dataset that contains CPS household level sequence (h_seq), individual participation indicator (snap_participation, 0 - not a recipient, 1 - current recipient on file, 2 - imputed recipient), and benefit amount.
# 
...
# CPS_dataset = pd.read_csv('/home/parker/Documents/AEI/Benefits/Housing/HCV_PBRA/cpsmar2014t.csv')
...
# convergence fails due to 'hpublic' variable. However, contributes to higher r-squared. Perfectly identifies those not receiving hlorent.
...
for FIPS in Admin_totals.Fips:
        ...
        if d['Difference in Population'][d['Fips'] == FIPS].values[0] < 0:
            ...
            f_House_yn.true_positive[pool.index[pool['true_positive']==1]] = 1
            ...
            this_state = (f_House_yn.f_gestfips==FIPS)
            f_House_yn.true_positive[this_state & current] = 1
...
print df['post augment CPS total benefits (annual)'].sum()
