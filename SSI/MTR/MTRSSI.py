...
def RandomForest(CPS, SSI_use, plot = False, sanity_check = False, MTR_Calc = True, adjustment = 10):
    ...
    if MTR_Calc == False:
        ...
        CPS['RF_MTR'] = pd.Series(np.zeros(len(CPS['earned_income'])))
        CPS.loc[ind, 'RF_MTR'] = MTR
        return MTR
...
# RF_MTR = RandomForest(CPS, CPS["ssi_val"],  plot = False, sanity_check = False, MTR_Calc = True, adjustment = 1)

# Now estimate using our calculated ssi instead:
RF_MTR = RandomForest(CPS, CPS["calc_SSI"], plot = False, sanity_check = False, MTR_Calc = True, adjustment = 10)
...
CPS['Income_rule_MTR'] = pd.Series(MTR_Program)
Program_MTR_zero = []
Program_MTR_positive = []
Program_MTR = [Program_MTR_zero, Program_MTR_positive]
CPS['regression_MTR'] = 0.
CPS_rec = CPS[CPS['current_recipient'] == 1].copy()
# MTR for income rules, regression, and Random Forests for individuals with no earned income:
df_zero = CPS_rec[(CPS_rec['earned_income'] == 0)]
slope, intercept = All_SM_SSI_reg(df_zero, plot = False, MTR_Calc = True)
Program_MTR[0].append(['income =' + str(0), \
    (np.sum(df_zero['marsupwt'] * df_zero['Income_rule_MTR'])/(np.sum(df_zero['marsupwt']))), (np.sum(df_zero['marsupwt'] * 
    	df_zero['RF_MTR'])/(np.sum(df_zero['marsupwt']))), slope])
CPS_rec.loc[df_zero.index.values, 'regression_MTR'] = slope

# MTR for income rules, regression, and Random Forests for single individuals with positive earned income:
df_single = CPS_rec[(CPS_rec['earned_income'] > 0) & (CPS_rec['a_spouse'] == 'None or children')]
slope, intercept = All_SM_SSI_reg(df_single, plot = False, MTR_Calc = True)
Program_MTR[1].append(['single and income >' + str(0), \
    (np.sum(df_single['marsupwt'] * df_single['Income_rule_MTR'])/(np.sum(df_single['marsupwt']))), (np.sum(df_single['marsupwt'] * 
    	df_single['RF_MTR'])/(np.sum(df_single['marsupwt']))), slope])
CPS_rec.loc[df_single.index.values, 'regression_MTR'] = slope

# MTR for income rules, regression, and Random Forests for married individuals with positive earned income:
df_married = CPS_rec[(CPS_rec['earned_income'] > 0) & (CPS_rec['a_spouse'] != 'None or children')]
slope, intercept = All_SM_SSI_reg(df_married, plot = False, MTR_Calc = True)
Program_MTR[1].append(['married and income >' + str(0), \
    (np.sum(df_married['marsupwt'] * df_married['Income_rule_MTR'])/(np.sum(df_married['marsupwt']))), (np.sum(df_married['marsupwt'] * 
    	df_married['RF_MTR'])/(np.sum(df_married['marsupwt']))), slope])
CPS_rec.loc[df_married.index.values,'regression_MTR'] = slope

CPS.loc[CPS_rec.index.values, 'regression_MTR'] = CPS_rec['regression_MTR']

# Outputting a .csv file with MTRs calculed using Random Forests and Income Rules:
CPS[['RF_MTR', 'Income_rule_MTR', 'regression_MTR', 'peridnum']].to_csv('SSI_MTR.csv', index=False)
