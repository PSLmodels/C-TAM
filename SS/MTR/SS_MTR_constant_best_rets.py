...
# [TODO] Add better regression, or random forest. Apply maximum earnings on previous and future earnings
# (perhaps interpolate into the future), 10 years minimum to qualify. Check the index wages to see if 
# they are right
...
def LE_reg(CPS, plot = False):
	...
	'''
	sample = CPS.copy()[(CPS['a_age'] >16) & (CPS['a_age'] < 66) & (CPS['a_ftpt'] == 0) & (CPS['earned_income'] > 0)]
...
CPS = CPS[[ 'AGEH' ,'AGES' ,'WAS', 'WASS','BIL_HEAD' ,'BIL_SPOUSE', 'FIL_HEAD', 'FIL_SPOUSE', 'HGA_HEAD', 'HGA_SPOUSE', 'FTPT_HEAD', 'FTPT_SPOUSE', 'FAMREL_HEAD','FAMREL_SPOUSE', 'MJIND_SPOUSE', 'MJIND_HEAD','CPSSEQ', 'WT']].fillna(0)
...
CPS_before = CPS.copy()
CPS_spouse = CPS[['earned_income_spouse','AGES', 'HGA_SPOUSE', 'FTPT_SPOUSE', 'FAMREL_SPOUSE', 'MJIND_SPOUSE', 'CPSSEQ','WT']][CPS.AGES != 0].copy()
CPS_spouse.columns = ['earned_income','a_age', 'a_hga', 'a_ftpt', 'a_famrel', 'a_mjind','CPSSEQ','wt']
CPS_head = CPS[['earned_income_head','AGEH', 'HGA_HEAD', 'FTPT_HEAD', 'FAMREL_HEAD', 'MJIND_HEAD', 'CPSSEQ','WT']].copy()
...
# CPS_laborforce = CPS_laborforce.loc[[7, 8],:]
...
final[['SS_MTR_head', 'SS_MTR_spouse', 'CPSSEQ']].to_csv('SS_MTR_ConstantFuture_RETS.csv', index = None)
