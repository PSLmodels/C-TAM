...
individuals in the 2014 CPS. We use a wage projection index in the 
...
def get_LE(x, age, wages, adjustment):
	...
	educ_level = ones * x 
	LE = np.exp(ones * params[0] + educ_level * params[1] + experience * params[2] + experienceSquared * params[3]).astype(int)#Using regression to determine LE
	if len(LE) == 0:
		LE = np.array([0])
	else:
...
def LE_reg(CPS, plot = False):
	...
	if plot == True:
		'''Plotting the fit of the regression to the data'''
...
def get_txt(sex, age, experience, peridnum, LE):
	...
	# Fourth line contains future wage index projection method (for this method)
	line081 = "081  0.002014"
...
# entry into the anypiab.exe SS benefit calculator.
